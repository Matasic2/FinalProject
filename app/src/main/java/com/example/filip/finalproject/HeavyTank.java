...
public class HeavyTank extends Units {
    ...
    HeavyTank(Context context, int x, int y, Player player) {
        ...
        if (player.color.equals("green")) {
            this.setParameters(GreenAttack1, GreenAttack2, GreenFirstAttackRange, GreenSecondAttackRange, GreenDefence, GreenHP, GreenHP, GreenMovement,8,greenAirAttack, healedBy);
        } else {
            this.setParameters(RedAttack1, RedAttack2, RedFirstAttackRange, RedSecondAttackRange, RedDefence, RedHP, RedHP, RedMovement,8,redAirAttack, healedBy);
        }
    }
}
